                                ...
                                noPar = FALSE )
{
  ...
  # Get biomass arrays
  SB_it        <- obj$om$SB_ispt[goodReps,sIdx,pIdx,]
  retroSB_itt  <- obj$mp$assess$retroSB_itspt[goodReps,,sIdx,pIdx,]
  ...
  for(i in 1:nReps)
  {
    for(t in 1:pT)
    {
      assErr_it[i,t] <- (retroSB_itt[i,t,tMP + t - 1] - SB_it[i,tMP + t - 1])/SB_it[i,tMP + t - 1]
    }
  }
  ...
  if(!noPar)
  {
    par(oma = c(3,3,1,1), mar = c(1,1,1,1))
  }
  ...
  plot( x = range(years[tIdx]),
        ...
        # Plot baseline
        polygon(  x = c(years[tIdx],rev(years[tIdx])),
                  y = c(assErr_qt[1,],rev(assErr_qt[3,])),
                  border = NA, col = "grey60")
        lines( x = years[tIdx], y = assErr_qt[2,],
                col = "black", lwd = 2)
        for( traceIdx in traces )
          lines(  x = years[tIdx], y = assErr_it[traceIdx,],
                  col = "black", lwd = .8 )
                                ...
                                redoEmpRefCurves = FALSE)
{
      ...
      redoEmpRefCurves )
  {
    ...
    uC_spk <- saveEmpRefCurves$uC_spk
    Y_spk  <- saveEmpRefCurves$Y_spk
    B_spk  <- saveEmpRefCurves$B_spk
    tB_spk <- saveEmpRefCurves$tB_spk
    F_spk  <- saveEmpRefCurves$F_spk
    U_spk  <- saveEmpRefCurves$U_spk
    M_spk  <- saveEmpRefCurves$M_spk
    R_spk  <- saveEmpRefCurves$R_spk
                          ...
                          traceSeed = 123 )
{
  ...
  } else B0_isp[,1,]  <- obj$ctlList$refPts$B0
