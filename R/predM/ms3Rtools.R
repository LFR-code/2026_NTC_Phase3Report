...
# calcFuncResponse()
# Takes per-capita predation rate and vulnerable
# prey biomass and estimates Holling disc equation
# parameters eta (encounter rate), h (handling time)
calcFuncResponse <- function( perCapPredRate_ft,
                              vulnB_ft, 
                              predB_ft,
                              etaMax_f,
                              hMax_f,
                              ...
                              typeII_f = rep(1,3) )
{
  ...
  # compile model
  compile("fitDisc.cpp",flags = "-g")
  # Dynamically link the C++ code
  dyn.load(dynlib("fitDisc"))     

  nF <- dim(perCapPredRate_ft)[1]   

  # Make data and par lists
  dat <- list(  pcP_ft        = perCapPredRate_ft,
                vB_ft         = vulnB_ft,
                P_ft          = predB_ft,
                etaMax_f      = etaMax_f,
                hMax_f        = hMax_f,
                fixTypeII_f   = typeII_f )

  # Convert init values to logit scale
  pEta_f <- etaInit_f/etaMax_f
  ph_f   <- hInit_f/hMax_f

  initLogitEta_f <- log(pEta_f/(1-pEta_f))
  initLogith_f <- log(ph_f/(1-ph_f))

  par <- list(  lneta_f     = initLogitEta_f,
                lnh_f       = initLogith_f,
                lnlambda_f  = rep(0,nF),
                lnsigma_f   = rep(log(sigmaRes),nF) )

  # Map - some predators will have
  # the same handling time, encounter rate, and curvature
  map <- list(  lneta_f     = factor(c(1:nF)),
                lnh_f       = factor(c(1:nF)),
                lnsigma_f   = factor(rep(NA,nF)) )

  maplnlambda_f  = 1:nF
  maplnlambda_f[typeII_f == 1] <- NA
  map$lnlambda_f <- factor(maplnlambda_f)

  # Estimate
  discFitObj <- TMB::MakeADFun(   data = dat,
                                  parameters = par,
                                  random= NULL,
                                  DLL= "fitDisc",
                                  map= map,
  ...
  tmbCtrl <- list(  eval.max = 1e4, 
                    iter.max = 1e4  )

  opt <- try( nlminb (  start     = discFitObj$par,
                          objective = discFitObj$fn,
                          gradient  = discFitObj$gr,
                          control   = tmbCtrl ) )
  # Check it out
  discFit <- discFitObj$report()

  discSD <- sdreport(discFitObj)

  dyn.unload(dynlib("fitDisc"))


  return(list(  discFit  = discFit,
                sdrepObj = discSD ))
} # END calcFuncResponse


# FATFRlogit()
# Foraging arena theory style functional response
# to derive predation mortality rate M = Q/Bprey from
# predator density Npred. Takes the Ahrens et al model for
# QB = vP/(k+P), and scales by a logistic function of 
# predator density to match the observed pattern in predation
# mortality rates, as well as adds a minimum QB value that
# helps with fitting to the data - however, model looks better when 
# qmin -> 0.
FATFRlogit <- function( P = seq(from = 0, to = 1e3, length.out = 1e3),
                        v = 0.08, k, lambda,
                        qmin  )
{
  QB <- qmin + ( (v*P - qmin) /(k + P) * 1/(1 + exp(-lambda * (P-k))))

  QB
} # END FATFRlogit

# FATFRlogitNLL()
# A negative log likelihood function for estimating
# parameters for the FATFRlogit() function. Can be
# used to fit, or just return parameters
FATFRlogitNLL <- function(  theta = c(0,0,0,0),
                            Pdata, QBdata,
                            sigmaQB = 0.01,
                            posPen = 0,
                            fit = TRUE )
{
  ...
  QBest    <- FATFRlogit( P = Pdata,
                          v = v, k = k, lambda = lambda, qmin = qmin)

  resids  <- (log(QBest) - log(QBdata))/sigmaQB

  objFun  <- sum(resids^2)

  if(v > max(QBdata))
    objFun <- objFun + posPen*(v - max(QBdata))^2
...
.saveBlob <- function( blob, ctlTable, outFolder )
{
  ...
  dir.create ( path )
  message( "\ (.saveBlob) Created simulation folder ",folder,"in ./Outputs/.\n" )
