...
.simPerfStats <- function( obj, inclAgg=FALSE, exclNoTAC=FALSE  )
{
  ...
  # speciesNames  <- om$speciesNames
  speciesNames  <- "Herring"
  stockNames    <- om$areaNames
  fYear         <- om$fYear
  ...
  B0_isp        <- array(NA, dim=c(nGoodReps,nS,(nP-1)))
  USR_isp       <- array(NA, dim=c(nGoodReps,nS,(nP-1)))
  
  # Spawning biomass
  SB_ispt   <- om$SB_ispt[allConvReps,,,,drop = FALSE]
  SB_ipt   <- apply(SB_ispt[,,,,drop=FALSE], FUN = sum, MARGIN = c(1,3,4))
  ...
  # Umsy_p   <- rp$FmsyRefPts$Umsy_p
  # MSY_p    <- rp$FmsyRefPts$YeqFmsy_p
  ...
  { 
    ...
    C_ipt  <- apply(C_ispft[,,,1:3,,drop=FALSE], FUN=sum, MARGIN=c(1,3,5))
    C_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    C_ispt[,1,,] <- C_ipt[,1:(nP-1),]

    # Ponded fish
    P_ispft      <- om$P_ispft[allConvReps,,,,,drop = FALSE]
    P_ipt       <- apply(P_ispft[,,,sokFleets,,drop=FALSE], FUN=sum, MARGIN=c(1,3,5))
    P_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    P_ispt[,1,,] <- P_ipt[,1:(nP-1),]

    # Dead ponded fish
    deadP_ipt <- apply(C_ispft[,,,sokFleets,,drop=FALSE], FUN=sum, MARGIN=c(1,3,5))
    deadP_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    deadP_ispt[,1,,] <- deadP_ipt[,1:(nP-1),]
    
    # SOK licenses
    L_ispft <- mp$hcr$sokEff_ispft[allConvReps,,,,,drop = FALSE]
    L_ipt  <- apply( X = L_ispft[,,,sokFleets,,drop = FALSE], FUN = sum, MARGIN = c(1,3,5))
    L_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    L_ispt[,1,,] <- L_ipt[,1:(nP-1),]

    # SOK product in kt
    psi_ispft    <- om$psi_ispft[allConvReps,,,,,drop = FALSE]
    SOK_ispft    <- P_ispft*psi_ispft
    SOK_ipt     <- apply(SOK_ispft[,,,sokFleets,,drop=FALSE], FUN=sum, MARGIN=c(1,3,5))
    SOK_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    SOK_ispt[,1,,] <- SOK_ipt[,1:(nP-1),]
    ...
    endSB_ipt <- apply(om$endSB_ispt[allConvReps,,,,drop = FALSE], FUN = sum, MARGIN = c(1,3,4))
    endSB_ipt[endSB_ipt == 0] <- NA
    endSB_ispt <- array(NA, dim = c(nGoodReps,nS,(nP-1),nT))
    endSB_ispt[,1,,] <- endSB_ipt[,1:(nP-1),]
    ...
    # Calculate VAR 5th and 95th percentiles for biomass and catch
    Bq_qspt <- apply(X = endSB_ispt[,,,tMP:nT,drop = FALSE], FUN = quantile, probs = c(0.05, 0.95), MARGIN = 2:4, na.rm = T)
    Cq_qspt <- apply(X = C_ispt[,,,tMP:nT,drop = FALSE], FUN = quantile, probs = c(0.05, 0.95), MARGIN = 2:4, na.rm = T)
    
    Bq_qsp <- apply(X = Bq_qspt, FUN = mean, MARGIN = c(1:3))
    Cq_qsp <- apply(X = Cq_qspt, FUN = mean, MARGIN = c(1:3))
    ...
    for( s in 1:nS )
      for(p in 1:(nP-1) )
      {
        ...
        statTable[rowIdx,"B5thpc"]          <- round(Bq_qsp[1,s,p], 3)
        statTable[rowIdx,"B95thpc"]         <- round(Bq_qsp[2,s,p], 3)
        ...
        statTable[rowIdx,"C5thpc"]          <- round(1e3*Cq_qsp[1,s,p], 3)
        statTable[rowIdx,"C95thpc"]         <- round(1e3*Cq_qsp[2,s,p], 3)
        ...
        statTable[rowIdx,"medB0"]           <- round(median(B0_isp[,s,p],na.rm = T),1)
        statTable[rowIdx,"mnB0"]            <- round(mean(B0_isp[,s,p],na.rm = T),1)
        statTable[rowIdx,"sdB0"]            <- round(sd(B0_isp[,s,p],na.rm = T),1)
        statTable[rowIdx,"l95B0"]           <- round(quantile(B0_isp[,s,p],na.rm = T,probs=0.025))
        statTable[rowIdx,"u95B0"]           <- round(quantile(B0_isp[,s,p],na.rm = T,probs=0.975))
    ...
    if(inclAgg)
    {
      ...
      aggRow[,"medB0"]              <- round(mean(B0_i,na.rm = T),3)
      aggRow[,"mnB0"]              <- round(mean(B0_i,na.rm = T),3)
